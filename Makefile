...
_opam/%: _opam/opam-init/init.sh ocaml-versions/%.json setup_sys_dune
	...
	opam update
	CC='gcc -Wa,-mbranches-within-32B' AS='as -mbranches-within-32B' OCAMLRUNPARAM="$(OCAML_RUN_PARAM)" OCAMLCONFIGOPTION="$(OCAML_CONFIG_OPTION)" opam switch create --keep-build-dir --yes $* ocaml-base-compiler.$*
